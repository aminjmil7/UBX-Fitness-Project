...
export class AlertErrorComponent implements OnDestroy {
  ...
  constructor(private alertService: AlertService, private eventManager: EventManager, translateService: TranslateService) {
    this.errorListener = eventManager.subscribe('ubxFitnessWebApp.error', (response: EventWithContent<unknown> | string) => {
      const errorResponse = (response as EventWithContent<AlertError>).content;
      this.addErrorAlert(errorResponse.message, errorResponse.key, errorResponse.params);
    });

    this.httpErrorListener = eventManager.subscribe('ubxFitnessWebApp.httpError', (response: EventWithContent<unknown> | string) => {
        ...
        case 400: {
          ...
          if (errorHeader) {
            ...
            for (const fieldError of fieldErrors) {
              ...
              const fieldName: string = translateService.instant(`ubxFitnessWebApp.${fieldError.objectName as string}.${convertedField}`);
